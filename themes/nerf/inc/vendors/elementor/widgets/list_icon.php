...
class Nerf_Elementor_List_Icon extends Widget_Base {
	...
	public function get_title() {
        return esc_html__( 'Apus List Icon', 'nerf' );
    }
        ...
        if ( !empty($features) ) {
            ...
            <div class="widget-list-icon <?php echo esc_attr($el_class); ?>">
                    <?php foreach ($features as $item): ?>
                            <div class="list-icon d-flex align-items-center <?php echo esc_attr($style); ?>">
                                    <?php
                                    ...
                                    if ( $item['image_icon'] == 'image' ) {
                                        if ( ! empty( $item['image']['url'] ) ) {
                                            ...
                                            $html .= '<div class="features-box-image flex-shrink-0 d-inline-flex align-items-center justify-content-center img">' . $image_html . '</div>';
                                        }
                                    } elseif ( $item['image_icon'] == 'icon' && !empty($item['icon'])) {
                                        $html .= '<div class="features-box-image flex-shrink-0 d-inline-flex align-items-center justify-content-center icon"><i class="'.$item['icon'].'"></i></div>';
                                    }
                                if ( $has_content ) {
                                    $html .= '<div class="box-content flex-grow-1">';
...
if ( version_compare(ELEMENTOR_VERSION, '3.5.0', '<') ) {
    Plugin::instance()->widgets_manager->register_widget_type( new Nerf_Elementor_List_Icon );
} else {
    Plugin::instance()->widgets_manager->register( new Nerf_Elementor_List_Icon );
}