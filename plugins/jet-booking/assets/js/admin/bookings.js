(function () {
	...
	Vue.component( 'jet-abaf-bookings-list', {
		...
		watch: {
			itemsList(value) {
				if (!this.detailsDialogOpened && this.urlParams.has('booking_details')) {
					this.currentItem = value.find(item => item.booking_id === this.urlParams.get('booking_details')) || false;
			...
			computedCurrentItem: {
				handler: function ( value, oldValue ) {
					...
					if (
						...
						|| ! moment( value.check_in_date ).isSame( +oldValue.check_in_date, 'day' )
						|| ! moment( value.check_out_date ).isSame( +oldValue.check_out_date, 'day' )
					) {
	...
	new Vue( {
		...
		created: function () {
			if ( this.urlParams.has( 'booking_details' ) ) {
				const bookingIndex = this.bookingsList.reverse().findIndex( item => item.booking_id === this.urlParams.get( 'booking_details' ) );
				const page = Math.ceil( bookingIndex / this.perPage ) || 1;
