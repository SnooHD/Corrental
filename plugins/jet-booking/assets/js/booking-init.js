(function () {
		...
		setDynamicPrice: function ( field = false ) {
			jQuery( 'span[data-price-change="1"][data-post="' + postID + '"]' ).each( function () {
				...
				let periodRange = JetBooking.createRange( period.start, period.end ),
					...
					priceList = [],
					priceRates = bacePrice.price_rates,
					ratePriceList,
					weekendPrices = bacePrice.weekend_price,
					weekendPriceList,
				...
				if (currentSeason.price) {
					...
					priceList.push(Number(bacePrice.price));
				}
	...
	const $bookingProductForm = $document.find('.jet-booking-form');
	...
	if ($bookingProductForm.length) {
		...
		$document.on('change.JetBookingProduct', function (event) {
