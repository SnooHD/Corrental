...
class Based {
	...
	public function __construct() {
		...
		// Add custom booking product price suffix.
		add_action( 'woocommerce_get_price_suffix', [ $this, 'add_price_suffix' ], 10, 4 );
		...
		new \JET_ABAF\WC_Integration\WC_Cart_Manager();
		new \JET_ABAF\WC_Integration\WC_Order_Manager();
	 ...
	 * Add price suffix.
	 *
	 * Add the suffix to display after prices > 0.
	 *
	 * @since  3.0.0
	 * @access public
	 *
	 ...
	 * @return mixed|string
	 */
	public function add_price_suffix( $html, $product, $price, $qty ) {

		if ( $this->is_booking_product( $product ) ) {
			$html .= jet_abaf()->engine_plugin->is_per_nights_booking() ? __( ' night', 'jet-booking' ) : __( ' day', 'jet-booking' );
		}
	...
	public function before_add_to_cart_button() {
		...
		jet_abaf()->engine_plugin->enqueue_deps( $product->get_id() );
