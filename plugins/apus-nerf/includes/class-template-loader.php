...
class Apus_Nerf_Template_Loader {
	...
	public static function templates( $template ) {
		...
		if ( !empty($wp_query->query_vars['post_type']) || $post_type ) {
			$custom_post_types = array( 'apartment' );
	 ...
	 * @throws Exception
	 */
	public static function locate( $name, $plugin_dir = APUS_NERF_PLUGIN_DIR ) {
		$template = '';

		$theme_folder_name = apply_filters( 'apus-nerf-theme-folder-name', 'apus-nerf' );
	 ...
	 * @throws Exception
	 */
	public static function get_template_part( $name, $args = array(), $plugin_dir = APUS_NERF_PLUGIN_DIR ) {
...
Apus_Nerf_Template_Loader::init();