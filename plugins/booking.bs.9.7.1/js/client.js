 ...
 */
function wpbc__inline_booking_calendar__days_css__get_rate(
  class_day,
  resource_id
) {
  if (
 ...
 */
function wpbc__inline_booking_calendar__days_css__get_season_names(
  class_day,
  resource_id
) {
  if (
...
function init_datepick_cal(
  ...
  if (jQuery("#calendar_booking" + bk_type).hasClass("hasDatepick") == true) {
    // If the calendar with the same Booking resource is activated already, then exist.
    return false;
  }

  var cl = document.getElementById("calendar_booking" + bk_type);
  ...
  function selectDay(date) {
    ...
    var is_calendar_booking_unselectable = jQuery(
      "#calendar_booking_unselectable" + bk_type
    ); //FixIn: 8.0.1.2
    var is_booking_form_also = jQuery("#booking_form_div" + bk_type);
    if (
      is_calendar_booking_unselectable.length > 0 &&
      is_booking_form_also.length <= 0
    ) {
  ...
  function hoverDay(value, date) {
    ...
    // Set unselectable,  if only Availability Calendar  here (and we do not insert Booking form by mistake).
    if (
      is_calendar_booking_unselectable.length == 1 &&
      is_booking_form_also.length != 1
    ) {
      jQuery(
        "#calendar_booking" + bk_type + " .datepick-days-cell-over"
      ).removeClass("datepick-days-cell-over"); // clear all highlight days selections
      jQuery(
        ".wpbc_only_calendar #calendar_booking" +
          bk_type +
          " .datepick-days-cell, " +
          ".wpbc_only_calendar #calendar_booking" +
          bk_type +
          " .datepick-days-cell div"
      ).css("cursor", "default");
      return false;
    } //FixIn: 8.0.1.2	end
  }
  ...
  function applyCSStoDays(date) {
    ...
    additional_class += wpbc__inline_booking_calendar__days_css__get_rate(
      class_day,
      bk_type
    ); // ' rate_100'
    additional_class +=
      wpbc__inline_booking_calendar__days_css__get_season_names(
        class_day,
        bk_type
      ); // ' weekend_season high_season'
    ...
    if (is_this_admin == false) {
      ...
      if (typeof wpbc_available_days_num_from_today !== "undefined")
        if (parseInt("0" + wpbc_available_days_num_from_today) > 0)
          if (
            days_between(date, my_test_date) >
            parseInt("0" + wpbc_available_days_num_from_today)
          )
            return [false, "cal4date-" + class_day + " date_user_unavailable"];
    }
    ...
    if (typeof is_this_day_available == "function") {
      ...
      if (!is_day_available) {
        return [
          ...
          "cal4date-" +
            class_day +
            " date_user_unavailable season_unavailable " +
            season_filter,
        ];
      }
    }
    ...
    if (typeof availability_per_day !== "undefined")
      if (typeof availability_per_day[bk_type] !== "undefined")
        if (typeof availability_per_day[bk_type][class_day] !== "undefined") {
          reserved_days_count = parseInt(
            availability_per_day[bk_type][class_day]
          );
        }
    ...
    if (typeof wpbc_check_in_dates !== "undefined")
      if (typeof wpbc_check_in_dates[bk_type] !== "undefined")
        if (typeof wpbc_check_in_dates[bk_type][class_day] !== "undefined") {
          ...
          checkin_days_count = [
            wpbc_check_in_dates[bk_type][class_day][0],
            wpbc_check_in_dates[bk_type][class_day][1],
          ];
        }
    ...
    if (typeof wpbc_check_out_dates !== "undefined")
      if (typeof wpbc_check_out_dates[bk_type] !== "undefined")
        if (typeof wpbc_check_out_dates[bk_type][class_day] !== "undefined") {
          ...
          checkout_days_count = [
            wpbc_check_out_dates[bk_type][class_day][0],
            wpbc_check_out_dates[bk_type][class_day][1],
          ];
        }
        ...
        if (typeof date2approve[bk_type] !== "undefined")
          if (typeof date2approve[bk_type][class_day] !== "undefined")
            return [
              ...
              "cal4date-" +
                class_day +
                " full_day_booking date2approve date_unavailable_for_all_childs " +
                blank_admin_class_day,
            ];
        return [
          false,
          "cal4date-" +
            class_day +
            " full_day_booking date_approved date_unavailable_for_all_childs " +
            blank_admin_class_day,
        ];
      }
    ...
    var is_set_pending_days_selectable = wpbc_settings.get_option(
          ...
          if (th == 0 && tm == 0 && ts == 0) {
            return [
              ...
              "cal4date-" +
                ...
                blank_admin_class_day,
            ]; // Orange       //FixIn: 8.6.1.18
          } else {
            if (is_booking_used_check_in_out_time === true) {
              if (ts == "1")
                additional_class +=
                  " check_in_time" + " check_in_time_date2approve"; //FixIn: 6.0.1.2
              if (ts == "2")
                additional_class +=
                  " check_out_time" + " check_out_time_date2approve"; //FixIn: 6.0.1.2
            } else {
              additional_class += " times_clock"; //FixIn: 8.2.1.27
            }
            time_return_value = [
              true,
              "date_available cal4date-" +
                class_day +
                " date2approve timespartly" +
                additional_class,
            ]; // Times
          ...
          if (th == 0 && tm == 0 && ts == 0)
            return [
              ...
              "cal4date-" +
                ...
                blank_admin_class_day,
            ]; //Blue or Grey in client
          else {
            if (is_booking_used_check_in_out_time === true) {
              if (ts == "1")
                additional_class +=
                  " check_in_time" + " check_in_time_date_approved"; //FixIn: 6.0.1.2
              if (ts == "2")
                additional_class +=
                  " check_out_time" + " check_out_time_date_approved"; //FixIn: 6.0.1.2
            } else {
              additional_class += " times_clock"; //FixIn: 8.2.1.27
            }
            time_return_value = [
    ...
    var is_exist_check_in_out_for_parent_resource = Math.max(
      checkin_days_count[0],
      checkout_days_count[0]
    );
    //FixIn: 8.4.7.29
    if (!wpdev_in_array(parent_booking_resources, bk_type)) {
    ...
    if (
      time_return_value !== false &&
      is_exist_check_in_out_for_parent_resource == 0
    ) {
      // Check  this only for single booking resources - is_exist_check_in_out_for_parent_resource == 0
      if (is_booking_used_check_in_out_time === true) {
        // If the date is check in/out and the check in/out time is activated so  then  this date is unavailable
        if (
          additional_class.indexOf("check_in_time") != -1 &&
          additional_class.indexOf("check_out_time") != -1
        ) {
      ...
      if (
        ...
        reserved_days_count - both_check_in_out_num <= 0
      ) {
        //FixIn: 6.0.1.2
        time_return_value[0] = false;
        time_return_value[1] = time_return_value[1].replace(
      ...
      if (
        ...
        if (reserved_days_count - both_check_in_out_num <= 0) {
          ...
          if (checkin_days_count[1] == 1)
            additional_class = " full_day_booking date_approved";
          else additional_class = " full_day_booking date2approve";
          return [
            false,
            "cal4date-" + class_day + additional_class + blank_admin_class_day,
          ];
        }

        // Recheck  if this date check in/out
        if (
          reserved_days_count - both_check_in_out_num - checkin_days_count[0] <=
          0
        ) {
          if (checkin_days_count[1] == 1)
            additional_class +=
              " date_approved check_in_time_date_approved"; //FixIn: 9.6.3.2
          else additional_class += " date2approve check_in_time_date2approve"; //FixIn: 9.6.3.2
          additional_class += " timespartly check_in_time";
        }
        if (
          reserved_days_count -
            both_check_in_out_num -
            checkout_days_count[0] <=
          0
        ) {
          if (checkout_days_count[1] == 1)
            additional_class +=
              " date_approved check_out_time_date_approved"; //FixIn: 9.6.3.2
  ...
  setTimeout(function () {
    jQuery(
      ".datepick-days-cell.datepick-today.datepick-days-cell-over"
    ).removeClass("datepick-days-cell-over");
  }, 500);
...
function get_first_day_of_selection(dates) {
  ...
  if (dates.indexOf(",") != -1) {
    ...
    for (var i = 0; i < length; i++) {
      ...
      new_dates_array[new_dates_array.length] =
        element[2] + "." + element[1] + "." + element[0]; //2013.12.20
    }
...
function setUnavailableSelectedDays(bk_type) {
  ...
  for (var i = 0; i < sel_dates.length; i++) {
    class_day2 =
      sel_dates[i].getMonth() +
      1 +
      "-" +
      sel_dates[i].getDate() +
      "-" +
      sel_dates[i].getFullYear();
    date_approved[bk_type][class_day2] = [
      sel_dates[i].getMonth() + 1,
      sel_dates[i].getDate(),
      sel_dates[i].getFullYear(),
      0,
      0,
    ...
    jQuery("#calendar_booking" + bk_type + " td.cal4date-" + class_day2).html(
      sel_dates[i].getDate()
    );
...
function setReservedSelectedDates(bk_type) {
  ...
  if (document.getElementById("calendar_booking" + bk_type) === null) {
    ...
    if (jQuery("#ajax_respond_insert" + bk_type).offset().top > 0) {
      //FixIn: 9.2.3.9
    ...
    if (
      document.getElementById("gateway_payment_forms" + bk_type) != null &&
    ...
    if (!is_pay_now || is_booking_without_payment == true)
      if (type_of_thank_you_message == "page") {
        ...
        document.getElementById("submiting" + bk_type).innerHTML =
          '<div class="submiting_content wpdev-help-message alert alert-warning alert-success" >' +
          new_booking_title +
          "</div>";

        if (parseInt(new_booking_title_time) > 0) {
          //FixIn: 9.6.2.2
    ...
    if (location.href.indexOf("admin.php") == -1) {
      ...
      jQuery("#calendar_booking" + bk_type).insertBefore(
        "#booking_form_div" + bk_type
      );
      document.getElementById("booking_form_div" + bk_type).style.display =
        "none";

      jQuery("#hided_booking_form" + bk_type)
        .prevAll('select[name="active_booking_form"]')
        .hide(); //FixIn: 7.1.2.13
      jQuery("#hided_booking_form" + bk_type)
        .prevAll('label[for="calendar_type"]')
        .hide();
        ...
        if (type_of_thank_you_message == "page") {
          ...
          if (!jQuery("#submiting" + bk_type).is(":visible")) {
            jQuery("#submiting" + bk_type)
              .closest("form.booking_form")
              .before(jQuery("#submiting" + bk_type));
...
function form_submit_send(
  ...
  document.getElementById("submiting" + bk_type).innerHTML =
    '<div style="height:20px;width:100%;text-align:center;margin:15px auto;"><img style="vertical-align:middle;box-shadow:none;width:14px;" src="' +
    wpdev_bk_plugin_url +
  ...
  if (document.getElementById("additional_calendars" + bk_type) != null) {
    id_additional_str = document.getElementById(
      "additional_calendars" + bk_type
    ).value; //Loop have to be here based on , sign
    ...
    if (!jQuery("#booking_form_div" + bk_type).is(":visible")) {
      jQuery("#booking_form_div" + bk_type).after(
        '<div class="wpbc_submit_spinner" style="height:20px;width:100%;text-align:center;margin:15px auto;"><img style="vertical-align:middle;box-shadow:none;width:14px;" src="' +
          wpdev_bk_plugin_url +
          '/assets/img/ajax-loader.gif"></div>'
      );
    }
    ...
    for (var ia = 0; ia < id_additional_arr.length; ia++) {
      ...
      for (var j = 0; j < formdata_additional_arr.length; j++) {
        ...
        if (my_form_field[1].substr(my_form_field[1].length - 2, 2) == "[]")
          my_form_field[1] =
            my_form_field[1].substr(
              0,
              my_form_field[1].length - ("" + bk_type).length - 2
            ) +
            id_additional +
            "[]";
        else
          my_form_field[1] =
            my_form_field[1].substr(
              0,
              my_form_field[1].length - ("" + bk_type).length
            ) + id_additional;

        formdata_additional +=
          my_form_field[0] + "^" + my_form_field[1] + "^" + my_form_field[2];
      }

      if (jQuery("#gateway_payment_forms" + bk_type).length > 0) {
        // If Payment form  for main  booking resources is showing then append payment form  for additional  calendars.
        jQuery("#gateway_payment_forms" + bk_type).after(
          '<div id="gateway_payment_forms' + id_additional + '"></div>'
        );
        jQuery("#gateway_payment_forms" + bk_type).after(
          '<div id="ajax_respond_insert' +
            id_additional +
...
function showErrorMessage(element, errorMessage, isScrollStop) {
  //FixIn: 8.5.1.3
  //FixIn: 8.4.0.2
  ...
  if (jQuery(element).attr("type") == "radio") {
    if (
      ...
      jQuery(element)
        ...
        .parent()
        .after(
          '<span class="wpdev-help-message wpdev-element-message alert alert-warning ' +
            message_type +
            '">' +
            errorMessage +
            "</span>"
...
jQuery(document).ready(function () {
  jQuery("div.inside_hint").on("click", function () {
    ...
    jQuery(this)
      ...
      .siblings(".has-inside-hint")
      .trigger("focus"); //FixIn: 8.7.11.12
  });

  jQuery("input.has-inside-hint")
    .on("blur", function () {
      //FixIn: 8.7.11.12
      if (this.value == "")
...
function wpbc_timeline_nav(timeline_obj, nav_step) {
  jQuery(".wpbc_timeline_front_end").trigger("timeline_nav", [
    timeline_obj,
    nav_step,
  ]); //FixIn:7.0.1.48
  ...
  jQuery(
    "#" +
      timeline_obj.html_client_id +
      " .wpbc_tl_prev,#" +
      timeline_obj.html_client_id +
      " .wpbc_tl_next"
  ).remove();

  jQuery("#" + timeline_obj.html_client_id + " .wpbc_tl_title").html(
    '<span class="glyphicon glyphicon-refresh wpbc_spin"></span> &nbsp Loading...'
  ); // '<div style="height:20px;width:100%;text-align:center;margin:15px auto;">Loading ... <img style="vertical-align:middle;box-shadow:none;width:14px;" src="'+wpdev_bk_plugin_url+'/assets/img/ajax-loader.gif"><//div>'
  ...
  jQuery.ajax({
    ...
    success: function (data, textStatus) {
      ...
      if (textStatus == "success") {
        jQuery(
          "#" + timeline_obj.html_client_id + " .wpbc_timeline_ajax_replace"
        ).html(data);
        return true;
      }
    },
    ...
    data: {
      ...
      wpbc_nonce: document.getElementById(
        "wpbc_nonce_" + timeline_obj.html_client_id
      ).value,
    },
  });
}
...
function bk_calendar_step_click(el) {
  ...
  if (br_id != undefined) {
    jQuery("#booking_form" + br_id + " .bk_calendar_step").css({
      display: "none",
    });
    jQuery("#booking_form" + br_id + " .bk_form_step").css({
      display: "block",
    });
  } else {
...
function wpbc_wizard_step(el, step_num, step_from) {
  ...
  if (
    wpbc_is_some_elements_visible(br_id, [
      "rangetime",
      "durationtime",
      "starttime",
      "endtime",
    ...
    if (
      wpbc_is_this_time_selection_not_available(
        br_id,
        document.getElementById("booking_form" + br_id)
      )
    ) {
      return false;
    }
  ...
  if (br_id != undefined) {
    jQuery("#booking_form" + br_id + " .wpbc_wizard_step").css({
      display: "none",
    });
    jQuery("#booking_form" + br_id + " .wpbc_wizard_step" + step_num).css({
      display: "block",
    });
  }
}
...
function wpbc_select_days_in_calendar(resource_id, selected_dates) {
  ...
  var inst = jQuery.datepick._getInst(
    document.getElementById("calendar_booking" + resource_id)
  );
  inst.dates = [];
  var original_array = [];
  var date;
  ...
  if (0) {
    // Select  one additional day in calendar, during editing of booking  //FixIn: 6.2.3.6
    var last_selected_date = new Date();
    last_selected_date.setFullYear(
      parseInt(selected_dates[selected_dates.length - 1][0])
    );
    last_selected_date.setMonth(
      parseInt(selected_dates[selected_dates.length - 1][1] - 1)
    );
    last_selected_date.setDate(
      parseInt(selected_dates[selected_dates.length - 1][2])
    );
    last_selected_date.setHours(0);
    last_selected_date.setMinutes(0);
    last_selected_date.setSeconds(0);
    var last_selected_next_date = new Date(
      last_selected_date.getTime() + 1000 * 60 * 60 * 24
    );
    selected_dates.push(
      new Array(
        last_selected_next_date.getFullYear(),
        last_selected_next_date.getMonth() + 1,
        last_selected_next_date.getDate()
      )
    );
  }
  ...
  for (var i = 0; i < selected_dates.length; i++) {
    ...
    original_array.push(
      jQuery.datepick._restrictMinMax(
        inst,
        jQuery.datepick._determineDate(inst, date, null)
      )
    ); //add date
    ...
    if (
      !wpdev_in_array(bk_distinct_dates, dta[2] + "." + dta[1] + "." + dta[0])
    )
  ...
  var dateStr =
    inst.dates.length == 0
      ? ""
      : jQuery.datepick._formatDate(inst, inst.dates[0]); // Get first date
  for (i = 1; i < inst.dates.length; i++)
    dateStr +=
      jQuery.datepick._get(inst, "multiSeparator") +
      jQuery.datepick._formatDate(inst, inst.dates[i]); // Gathering all dates
  jQuery("#date_booking" + resource_id).val(dateStr); // Fill the input box

  if (original_array.length > 0) {
    // Set showing of start month
  ...
  if (typeof check_condition_sections_in_bkform == "function") {
    check_condition_sections_in_bkform(
