...
function wpbc_booking_save( $request_params ){
	...
	if ( function_exists( 'wpbc_maybe_get_payment_form' ) ) {
		...
		if (
			...
			&& ( ! empty( $payment_params['booked_dates_times_arr']) )
			&& ( ! empty($payment_params['booked_dates_times_arr']['dates_ymd_arr']) )
			&& ( count( $payment_params['booked_dates_times_arr']['dates_ymd_arr'] ) > 1 )
		) {
			unset( $payment_params['booked_dates_times_arr']['dates_ymd_arr'][ ( count( $payment_params['booked_dates_times_arr']['dates_ymd_arr'] ) - 1 ) ] );   
	

			$str_dates__dd_mm_yyyy = wpbc_convert_dates_arr__yyyy_mm_dd__to__dd_mm_yyyy( $payment_params['booked_dates_times_arr']['dates_ymd_arr'] ); // ['2023-10-20','2023-10-25'] => ['20.10.2023','25.10.2023']
			$payment_params['str_dates__dd_mm_yyyy'] = implode( ',', $str_dates__dd_mm_yyyy ); 
