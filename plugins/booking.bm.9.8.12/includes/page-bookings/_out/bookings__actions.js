...
function wpbc_ajx_booking_ajax_action_request() {
  ...
  if (
    undefined != action_param["booking_id"] &&
    !Array.isArray(action_param["booking_id"])
  ) {
    // Not array
    action_param["locale"] = wpbc_get_selected_locale(
      action_param["booking_id"],
      wpbc_ajx_booking_listing.get_secure_param("locale")
    );

    const item = document.querySelector(
      `#row_id_${action_param["booking_id"]}`
    );

    const alteredLang = item
      .querySelector(".ui_element_locale select")
      .getAttribute("value_of_selected_option");
    const lang = alteredLang || item.getAttribute("data-lang");

    if (lang) {
      action_param["locale"] = lang;
    }
  }
  ...
  var action_post_params = {
    ...
    wpbc_ajx_user_id:
      undefined == action_param["user_id"]
        ? wpbc_ajx_booking_listing.get_secure_param("user_id")
        : action_param["user_id"],
    wpbc_ajx_locale:
      undefined == action_param["locale"]
        ? wpbc_ajx_booking_listing.get_secure_param("locale")
        : action_param["locale"],
  ...
  jQuery
    .post(
      ...
      function (response_data, textStatus, jqXHR) {
        console.log(
          " == Ajax Actions :: Response WPBC_AJX_BOOKING_ACTIONS == ",
          response_data
        );
        ...
        if (_typeof(response_data) !== "object" || response_data === null) {
          ...
          jQuery(
            wpbc_ajx_booking_listing.get_other_param("listing_container")
          ).html(
        ...
        wpbc_admin_show_message(
          ...
          "undefined" ===
            typeof response_data["ajx_after_action_result_all_params_arr"][
              "after_action_result_delay"
            ]
            ? 10000
            : response_data["ajx_after_action_result_all_params_arr"][
                "after_action_result_delay"
              ]
        ); // Success response
        ...
        if ("1" == response_data["ajx_after_action_result"]) {
          ...
          if (
            ...
            wpbc_ajx_booking__export_csv_url__download(
              response_data["ajx_after_action_result_all_params_arr"][
                "export_csv_url"
              ]
            );
        ...
        wpbc_button__remove_spin(
          response_data["ajx_cleaned_params"]["ui_clicked_element_id"]
        ); // Hide modals
  ...
  // .always( function ( data_jqXHR, textStatus, jqXHR_errorThrown ) {   if ( window.console && window.console.log ){ console.log( 'always finished', data_jqXHR, textStatus, jqXHR_errorThrown ); }     }) // End Ajax
}
/**
...
function wpbc_ajx_click_on_dates_toggle(this_date) {
  jQuery(this_date)
    .parents(".wpbc_col_dates")
    .find(".booking_dates_small")
    .toggle();
  jQuery(this_date)
    .parents(".wpbc_col_dates")
    .find(".booking_dates_full")
    .toggle();
  /*
...
function wpbc_ajx_booking__ui_define__locale() {
  jQuery(".wpbc_listing_container select").each(function (index) {
    ...
    if (undefined !== selection) {
      ...
      if (
        "" != selection &&
        jQuery(this).hasClass("set_booking_locale_selectbox")
      ) {
        // Locale
        var booking_locale_button = jQuery(this)
          .parents(".ui_element_locale")
          .find(".set_booking_locale_button"); //booking_locale_button.css( 'color', '#db4800' );		// Set button  red
...
function wpbc_ajx_booking__ui_define__remark() {
  jQuery(".wpbc_listing_container .ui_remark_section textarea").each(function (
    index
  ) {
    var text_val = jQuery(this).val();

    if (undefined !== text_val && "" != text_val) {
      var remark_button = jQuery(this)
        .parents(".ui_group")
        .find(".set_booking_note_button");
...
function wpbc_ajx_booking__ui_click_show__change_resource(
  booking_id,
  resource_id
) {
  ...
  jQuery("#change_booking_resource__resource_select")
    .val(resource_id)
    .trigger("change");
  ...
  cbr.appendTo(
    jQuery("#ui__change_booking_resource__section_in_booking_" + booking_id)
  );
  ...
  if (
    !jQuery(
      "#ui__change_booking_resource__section_in_booking_" + booking_id
    ).is(":visible")
  ) {
...
function wpbc_ajx_booking__ui_click_save__change_resource(
  this_el,
  booking_action,
  el_id
) {
  wpbc_ajx_booking_ajax_action_request({
    ...
    selected_resource_id: jQuery(
      "#change_booking_resource__resource_select"
    ).val(),
...
function wpbc_ajx_booking__ui_click_show__duplicate_booking(
  booking_id,
  resource_id
) {
  // Define ID of booking to hidden input
  ...
  jQuery("#duplicate_booking_to_other_resource__resource_select")
    .val(resource_id)
    .trigger("change");
  var cbr; // Get Resource section
  ...
  cbr.appendTo(
    jQuery(
      "#ui__duplicate_booking_to_other_resource__section_in_booking_" +
        booking_id
    )
  );
  ...
  jQuery(
    "#ui__duplicate_booking_to_other_resource__section_in_booking_" + booking_id
  ).toggle();
}

function wpbc_ajx_booking__ui_click_save__duplicate_booking(
  this_el,
  booking_action,
  el_id
) {
  wpbc_ajx_booking_ajax_action_request({
    booking_action: booking_action,
    booking_id: jQuery(
      "#duplicate_booking_to_other_resource__booking_id"
    ).val(),
    selected_resource_id: jQuery(
      "#duplicate_booking_to_other_resource__resource_select"
    ).val(),
    ui_clicked_element_id: el_id,
  });
  wpbc_button_enable_loading_icon(this_el); // wpbc_ajx_booking__ui_click_close__change_resource();
...
function wpbc_ajx_booking__ui_click_close__duplicate_booking() {
  ...
  cbrce.appendTo(
    jQuery("#wpbc_hidden_template__duplicate_booking_to_other_resource")
  );
  cbrce = null; // Hide all change booking resources sections
...
function wpbc_ajx_booking__ui_click_show__set_payment_status(booking_id) {
  var jSelect = jQuery(
    "#ui__set_payment_status__section_in_booking_" + booking_id
  ).find("select");
  ...
  if (!isNaN(parseFloat(selected_pay_status))) {
    ...
    jSelect
      .find('option[value="' + selected_pay_status + '"]')
      .prop("selected", true); // Otherwise known payment status
  } // Hide sections of "Change booking resource" in all other bookings ROWs

  if (
    !jQuery("#ui__set_payment_status__section_in_booking_" + booking_id).is(
      ":visible"
    )
  ) {
    jQuery(".ui__under_actions_row__section_in_booking").hide();
...
function wpbc_ajx_booking__ui_click_save__set_payment_status(
  booking_id,
  this_el,
  booking_action,
  el_id
  ...
  wpbc_ajx_booking_ajax_action_request({
    ...
    selected_payment_status: jQuery(
      "#ui_btn_set_payment_status" + booking_id
    ).val(),
...
function wpbc_ajx_booking__ui_click_save__set_booking_cost(
  booking_id,
  this_el,
  booking_action,
  el_id
  ...
  wpbc_ajx_booking_ajax_action_request({
    ...
    booking_id: booking_id,
    booking_cost: jQuery(
      "#ui_btn_set_booking_cost" + booking_id + "_cost"
    ).val(),
    ui_clicked_element_id: el_id + "_save",
  });
  wpbc_button_enable_loading_icon(this_el);
...
function wpbc_ajx_booking__ui_click__send_payment_request() {
  wpbc_ajx_booking_ajax_action_request({
    ...
    reason_of_action: jQuery(
      "#wpbc_modal__payment_request__reason_of_action"
    ).val(),
    ui_clicked_element_id: "wpbc_modal__payment_request__button_send",
  });
  wpbc_button_enable_loading_icon(
    jQuery("#wpbc_modal__payment_request__button_send").get(0)
  );
}
/**
...
function wpbc_ajx_booking__ui_click__import_google_calendar() {
  wpbc_ajx_booking_ajax_action_request({
    ...
    booking_gcal_events_from: jQuery(
      "#wpbc_modal__import_google_calendar__section #booking_gcal_events_from option:selected"
    ).val(),
    booking_gcal_events_from_offset: jQuery(
      "#wpbc_modal__import_google_calendar__section #booking_gcal_events_from_offset"
    ).val(),
    booking_gcal_events_from_offset_type: jQuery(
      "#wpbc_modal__import_google_calendar__section #booking_gcal_events_from_offset_type option:selected"
    ).val(),
    booking_gcal_events_until: jQuery(
      "#wpbc_modal__import_google_calendar__section #booking_gcal_events_until option:selected"
    ).val(),
    booking_gcal_events_until_offset: jQuery(
      "#wpbc_modal__import_google_calendar__section #booking_gcal_events_until_offset"
    ).val(),
    booking_gcal_events_until_offset_type: jQuery(
      "#wpbc_modal__import_google_calendar__section #booking_gcal_events_until_offset_type option:selected"
    ).val(),
    booking_gcal_events_max: jQuery(
      "#wpbc_modal__import_google_calendar__section #booking_gcal_events_max"
    ).val(),
    booking_gcal_resource: jQuery(
      "#wpbc_modal__import_google_calendar__section #wpbc_booking_resource option:selected"
    ).val(),
  });
  wpbc_button_enable_loading_icon(
    jQuery(
      "#wpbc_modal__import_google_calendar__section #wpbc_modal__import_google_calendar__button_send"
    ).get(0)
  );
}
/**
