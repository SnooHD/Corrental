...
function do_console_log($prop){
		?>
		<script type="text/javascript">
				console.log(`<?php json_encode(print_r($prop)); ?>`);
		</script>
		<?php
...
// Check  if this date available for specific booking resource, depend on from the season filter.
function wpbc_is_this_day_available_in_season_filters( $date, $bk_type, $season_filters = array() ){
...
function wpbc_initiate_season_filter_cache(){
	...
	//This function  must exist,  it's valid function!!!
	/**
		 * Check if this day inside of filter  , return TRUE  or FALSE   or   array( 'hour', 'start_time', 'end_time']) if HOUR filter this FILTER ID
	 *
	 * @param int $day
	    ...
	    if ( isset( $filter ) > 0 ) {
	        ...
	        if ( (isset( $filter['version'] )) && ($filter['version'] == '2.0') ) { // Ver: 2.0
	            ...
	            if ( ( ! empty( $filter['start_time'] )) && ( ! empty( $filter['end_time'] )) ) {
	               ...
	               return true; //deprecated: return array( 'hour', $filter['start_time'], $filter['end_time'] );             // Its hourly filter, so its apply to all days
	            }
	...
	/**
	 * Check  if "Check In" day in specific season
	 *
	 * @param $season_filter_id
	 * @param $days_string
	 *
	 * @return bool
	 */
	function wpbc_is_check_in_day_in_season_filter( $season_filter_id, $days_string ) {

		if ( $season_filter_id == '0' ) {
			return true;												// All days - not the season  filter
		}
		...
		$is_day_inside_of_filter = wpbc_is_day_inside_of_filter(
																	$check_in['day'],
																	$check_in['month'],
																	$check_in['year'],
																	$season_filter_id
																);
		return $is_day_inside_of_filter;
	}
...
//debuge($param);
    ...
    ////////////////////////////////////////////////////////////////
    // Extend unavailable interval to extra hours; cleaning time, or any other service time
    ////////////////////////////////////////////////////////////////
    /*            
    $extra_hours_in  = 1;
    $extra_hours_out = 1;
...
/**
 * Get additional booking data array  for each  dates
 *
 * @param int $resource_id							- ID  of booking resource	Default: 1
 * @param bool $only_for_check_out_dates			- Check out or all dates.	Default: false.			{ true - check out dates, 	false - All  dates }
 *
 * @return array
 *
 *            Example:
 ...
 *              				, ...
*/
function wpbc_get_additional_booking_info_to_dates_arr( $resource_id = 1 , $only_for_check_out_dates = false ) {
    ...
    $args = array(
	    'wh_booking_type'       => intval( $resource_id ),
	    'wh_approved'           => '',
	    'wh_booking_id'         => '',
	    'wh_is_new'             => '',
	    'wh_pay_status'         => 'all',
	...
	foreach ( $tip_bookings as $booking_id => $booking_data ) {
		...
		foreach ( $booking_data->dates as $bk_check_date ) {
			...
			if ( ( ! $only_for_check_out_dates ) || ( 2 == $is_it_check_out ) ) {

				// Get booking details hint relative settings
				if ( function_exists( 'get_title_for_showing_in_day' ) ) {
					$booking_data_hint = esc_textarea( get_title_for_showing_in_day( $booking_id, $tip_bookings, get_bk_option( 'booking_booked_data_in_tooltips' ) ) );
				} else {
					$booking_data_hint = '';
				}

				// Add to array with dates booking text data - only  to  check  out dates.
				...
				if ( ! isset( $dates_additional_info[ $my_day_tag ] ) ) {
					$dates_additional_info[ $my_day_tag ] = array();
				}
				$dates_additional_info[ $my_day_tag ][ $my_time_in_minutes ] = $booking_data_hint;
			}
		}
	}

	return $dates_additional_info;
}
