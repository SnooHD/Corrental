	 ...
	 * Get array of booking resources Objects
        ...
        function wpbc_db__get_resource_title( $resource_id = 1){
	        ...
	        if ( $types_list ) {
				...
				function wpbc_db_get_resource_id_by_title( $title ){
					...
					global $wpdb;
					$types_list  = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}bookingtypes WHERE title = '{$title}'"); //  WHERE title = {$title}" );
	        if ( isset($types_list[0]) && $types_list[0]->booking_type_id) {
		        return $types_list[0]->booking_type_id;		//FixIn: 8.8.3.17
	        }  else {
		        return false;
	        }
        }
