...
(function ($) {
  ...
  $.extend(Datepick.prototype, {
    ...
    _attachDatepick: function (target, settings) {
      if (!target.id) target.id = "dp" + ++this._uuid;
      var nodeName = target.nodeName.toLowerCase();
      var inst = this._newInst(
        $(target),
      ...
      if (inst.inline) {
        inst.dpDiv.addClass(
          this._inlineClass[this._get(inst, "useThemeRoller") ? 1 : 0]
        );
        this._inlineDatepick(target, inst);
      } else this._connectDatepick(target, inst);
    },
    ...
    _connectDatepick: function (target, inst) {
      ...
      if (showOn == "button" || showOn == "both") {
        ...
        var trigger = $(
          this._get(inst, "buttonImageOnly")
            ? $("<img/>")
                .addClass(this._triggerClass[useTR])
      ...
      input
        ...
        .on("keyup", this._doKeyUp); //FixIn: 8.7.11.12
    ...
    _dialogDatepick: function (input, date, onSelect, settings, pos) {
      ...
      this._inDialog = true;
      this.dpDiv.addClass(
        this._dialogClass[this._get(inst, "useThemeRoller") ? 1 : 0]
      );
    ...
    _destroyDatepick: function (target) {
      ...
      if (inst.inline) $target.removeClass(this.markerClassName).empty();
      else {
    ...
    _enableDatepick: function (target) {
      ...
      if (inst.inline)
        $target
          .children("." + this._disableClass[useTR])
          .remove()
          .end()
          .find("select")
          .attr("disabled", "");
        ...
        inst.siblings
          .filter("button." + this._triggerClass[useTR])
          .each(function () {
            this.disabled = false;
          })
          .end()
    ...
    _optionDatepick: function (target, name, value) {
      ...
      if (inst) {
        ...
        if (!blank) this._setDateDatepick(target, dates);
        if (inst.inline)
          $(target)
            .children("div")
            .removeClass(this._inlineClass.join(" "))
            .addClass(
              this._inlineClass[this._get(inst, "useThemeRoller") ? 1 : 0]
            );
        this._updateDatepick(inst);
      ...
      if ($.datepick._datepickerShowing)
        switch (event.keyCode) {
          ...
          case 13:
            ...
            if (sel.length == 0)
              sel = $(
                "td." + $.datepick._selectedClass[useTR] + ":first",
                inst.dpDiv
              );
            if (sel[0])
              $.datepick._selectDay(
    ...
    _doMouseOver: function (td, id, timestamp) {
      ...
      if ($(td).text()) {
        ...
        if ($.datepick._get(inst, "showStatus")) {
          var status =
            $.datepick
              ._get(inst, "statusForDate")
              .apply(inst.input ? inst.input[0] : null, [date, inst]) ||
    ...
    _doMouseOut: function (td, id) {
      ...
      $(td)
        ...
        .removeClass($.datepick._weekOverClass[useTR]);
      if ($.datepick._get(inst, "showStatus"))
        $("#" + $.datepick._statusId[useTR] + id).html(
          $.datepick._get(inst, "initStatus")
        );
      if ($.datepick._get(inst, "onHover")) $.datepick._doHover(td, "#" + id);
    },
    ...
    _doHover: function (td, id, year, month) {
      ...
      //FixIn: 2023-08-06
      var temp_daylight_day = $(td).find("a").length
        ? new Date(year, month, $(td).find("a").text())
        : null;
      temp_daylight_day =
        null === temp_daylight_day && $(td).find("span").length
          ? new Date(year, month, $(td).find("span").text())
          : temp_daylight_day;

      var date = year
        ? //			this._daylightSavingAdjust(new Date(year, month, $(td).find('a').text())) : null);							//FixIn: 2023-08-06
          this._daylightSavingAdjust(temp_daylight_day)
        : null; //FixIn: 2023-08-06
      onHover.apply(inst.input ? inst.input[0] : null, [
        date ? this._formatDate(inst, date) : "",
        date,
        inst,
      ]);
    },
    ...
    _showDatepick: function (input) {
      ...
      if (!inst.inline) {
        ...
        var postProcess = function () {
          ...
          inst.dpDiv
            .find("iframe." + $.datepick._coverClass[useTR]) // IE6- only
            .css({
              left: -borders[0],
              top: -borders[1],
        ...
        if (inst.input[0].type != "hidden") $(inst.input).trigger("focus"); //FixIn: 8.7.11.12
    ...
    _updateDatepick: function (inst) {
      ...
      if (!inst.inline) inst.dpDiv.attr("id", this._mainDivId[useTR]);
      ...
      inst.dpDiv
        .removeClass(this._mainDivClass[1 - useTR])
        .addClass(this._mainDivClass[useTR])
        .removeClass(this._multiClass.join(" "))
        .addClass(
          numMonths[0] != 1 || numMonths[1] != 1 ? this._multiClass[useTR] : ""
        )
    ...
    _getBorders: function (elem) {
      var convert = function (value) {
        ...
        return (
          { thin: 1 + extra, medium: 3 + extra, thick: 5 + extra }[value] ||
          value
        );
      };
      return [
    ...
    _checkOffset: function (inst, offset, isFixed) {
      ...
      var above =
        ...
        (isFixed && $.browser_is_supported_datepick.opera
      ...
      var alignR =
        pos[0] +
        (inst.input ? inst.input.outerWidth() : 0) -
        inst.dpDiv.outerWidth() -
        (isFixed && $.browser_is_supported_datepick.opera
      ...
      var tooHigh =
        offset.top + inst.dpDiv.outerHeight() - scrollY > browserHeight;
    ...
    _hideDatepick: function (input, duration) {
      ...
      if (!inst || (input && inst != $.data(input, PROP_NAME))) return false;
      var rangeSelect = this._get(inst, "rangeSelect");
      if (rangeSelect && inst.stayOpen) this._updateInput("#" + inst.id);
      inst.stayOpen = false;
    ...
    _checkExternalClick: function (event) {
      ...
      var useTR = $.datepick._get($.datepick._curInst, "useThemeRoller")
        ? 1
        : 0;
      if (
        !$target
          .parents()
          .andSelf()
          .is("#" + $.datepick._mainDivId[useTR]) &&
        !$target.hasClass($.datepick.markerClassName) &&
        !$target
          .parents()
          .andSelf()
          .hasClass($.datepick._triggerClass[useTR]) &&
    ...
    _gotoToday: function (id) {
      ...
      else inst.cursorDate = this._daylightSavingAdjust(new Date());
      inst.drawMonth = inst.cursorDate.getMonth();
      inst.drawYear = inst.cursorDate.getFullYear();
    ...
    _selectMonthYear: function (id, select, period) {
      ...
      inst.cursorDate.setDate(
        Math.min(
          inst.cursorDate.getDate(),
    ...
    _clickMonthYear: function (id) {
      ...
      if (
        inst.input &&
        inst.selectingMonthYear &&
        !$.browser_is_supported_datepick.msie
      )
        $(inst.input).trigger("focus"); //FixIn: 8.7.11.12
    ...
    _selectDay: function (td, id, timestamp) {
      var inst = this._getInst($(id)[0]);
      var useTR = this._get(inst, "useThemeRoller") ? 1 : 0;
      if ($(td).hasClass("check_in_time") && inst.dates.length !== 1)
        return false;
      if ($(td).hasClass(this._unselectableClass[useTR])) return false;
      var rangeSelect = this._get(inst, "rangeSelect");
      var multiSelect = this._get(inst, "multiSelect");
      if (rangeSelect) inst.stayOpen = !inst.stayOpen;
      else if (multiSelect) inst.stayOpen = true;
      if (inst.stayOpen) {
      ...
      if (rangeSelect && !inst.stayOpen) {
        if (inst.dates[0].getTime() === date.getTime()) {
          this._clearDate(id);
          return false;
        } else {
          inst.dates[1] = date;
        ...
        if (pos > -1) inst.dates.splice(pos, 1);
        else if (inst.dates.length < multiSelect) {
          if (inst.dates[0]) inst.dates.push(date);
          else inst.dates = [date];
      ...
      if (inst.stayOpen) this._updateDatepick(inst);
    ...
    _clearDate: function (id) {
      var target = $(id);
      var inst = this._getInst(target[0]);
      if (this._get(inst, "mandatory")) return false;
      inst.stayOpen = false;
      inst.clear = true;
    ...
    _updateInput: function (id) {
      ...
      if (inst.inline) this._updateDatepick(inst);
        ...
        if (typeof inst.input[0] != "object") $(inst.input).trigger("focus"); //FixIn: 8.7.11.12	// Restore focus
        this._lastInput = null;
      }
      return false;
    },
    ...
    _showDate: function (inst) {
      ...
      if (inst.input) {
        dateStr =
          inst.dates.length == 0 ? "" : this._formatDate(inst, inst.dates[0]);
        if (dateStr) {
    ...
    formatDate: function (format, date, settings) {
      ...
      var monthNames = settings.monthNames || this._defaults.monthNames;
      var calculateWeek =
        settings.calculateWeek || this._defaults.calculateWeek;
      // Check whether a format character is doubled
      var lookAhead = function (match) {
        var matches =
          iFormat + 1 < format.length && format.charAt(iFormat + 1) == match;
    ...
    _setDateFromField: function (inst) {
      ...
      dates = rangeSelect
        ? dates.split(this._get(inst, "rangeSeparator"))
        : multiSelect
        ? dates.split(this._get(inst, "multiSeparator"))
        : [dates];
      inst.dates = [];
      var settings = this._getFormatConfig(inst);
      for (var i = 0; i < dates.length; i++)
      ...
      for (var i = inst.dates.length - 1; i >= 0; i--)
        if (!inst.dates[i]) inst.dates.splice(i, 1);
      if (rangeSelect && inst.dates.length < 2) inst.dates[1] = inst.dates[0];
      if (multiSelect && inst.dates.length > multiSelect)
        inst.dates.splice(multiSelect, inst.dates.length);
      inst.cursorDate = new Date(
        (inst.dates[0] || this._getDefaultDate(inst)).getTime()
      );
      inst.drawMonth = inst.cursorDate.getMonth();
      inst.drawYear = inst.cursorDate.getFullYear();
    ...
    _determineDate: function (inst, date, defaultDate) {
      ...
      date =
        date == null
          ...
          ? offsetString(date)
          : typeof date == "number"
          ? isNaN(date) || date == Infinity || date == -Infinity
            ? defaultDate
            : offsetNumeric(date)
          : date;
      date =
        date && (date.toString() == "Invalid Date" || date.toString() == "NaN")
          ? defaultDate
          : date;
      if (date) {
        date.setHours(0);
    ...
    _setDate: function (inst, date, endDate) {
      ...
      inst.dates = [
        this._restrictMinMax(
          inst,
          this._determineDate(inst, date[0], new Date())
        ),
      ];
      inst.cursorDate = new Date(inst.dates[0].getTime());
      inst.drawMonth = inst.cursorDate.getMonth();
      ...
      if (this._get(inst, "rangeSelect"))
        inst.dates[1] =
          date.length < 1
            ...
            : this._restrictMinMax(
                inst,
                this._determineDate(inst, date[1], null)
              );
      else if (this._get(inst, "multiSelect"))
        for (var i = 1; i < date.length; i++)
          inst.dates[i] = this._restrictMinMax(
            inst,
            this._determineDate(inst, date[i], null)
          );
      if (
        origMonth != inst.cursorDate.getMonth() ||
        origYear != inst.cursorDate.getFullYear()
      )
        this._notifyChange(inst);
    ...
    _getDate: function (inst) {
      var startDate =
        inst.input && inst.input.val() == "" ? null : inst.dates[0];
      if (this._get(inst, "rangeSelect"))
        return startDate
          ? [inst.dates[0], inst.dates[1] || inst.dates[0]]
          : [null, null];
      else if (this._get(inst, "multiSelect"))
    ...
    _generateHTML: function (inst) {
      ...
      var clear = this._get(inst, "mandatory")
        ? ""
        : '<div class="' +
          this._clearClass[useTR] +
          '"><a href="javascript:void(0)" ' +
          "onclick=\"jQuery.datepick._clearDate('#" +
          inst.id +
          "');\"" +
          this._addStatus(
            useTR,
      ...
      var controls =
        '<div class="' +
        this._controlClass[useTR] +
        '">' +
        (isRTL ? "" : clear) +
        '<div class="' +
        this._closeClass[useTR] +
        '"><a href="javascript:void(0)" ' +
      ...
      var gotoDate =
        this._get(inst, "gotoCurrent") && inst.dates[0] ? inst.dates[0] : today;
      ...
      for (var row = 0; row < numMonths[0]; row++) {
        for (var col = 0; col < numMonths[1]; col++) {
          ...
          html +=
            '<div class="' +
            this._oneMonthClass[useTR] + // Responsive skin
            ...
            '<table class="' +
            this._tableClass[useTR] +
            '" cellpadding="0" cellspacing="0"><thead>' +
            '<tr class="' +
            this._tableHeaderClass[useTR] +
            '">' +
            (showWeeks
              ? "<th" +
                this._addStatus(
          ...
          for (var dow = 0; dow < 7; dow++) {
            ...
            var dayStatus =
              !showStatus || !changeFirstDay
                ? ""
                : status
                    .replace(/DD/, dayNames[day])
                    .replace(/D/, dayNamesShort[day]);
            html +=
              "<th" +
              ((dow + firstDay + 6) % 7 < 5
                ? ""
          ...
          if (
            drawYear == inst.cursorDate.getFullYear() &&
            drawMonth == inst.cursorDate.getMonth()
          )
            inst.cursorDate.setDate(
              Math.min(inst.cursorDate.getDate(), daysInMonth)
            );
          var leadDays =
          ...
          for (var dRow = 0; dRow < numRows; dRow++) {
            ...
            html +=
              '<tr class="' +
              this._weekRowClass[useTR] +
              '">' +
              (showWeeks
                ? '<td class="' +
                  this._weekColClass[useTR] +
                  '"' +
                  this._addStatus(
                    useTR,
                    showStatus,
            ...
            for (var dow = 0; dow < 7; dow++) {
              ...
              var selected =
                ...
                printDate.getTime() <=
                  (inst.dates[1] || inst.dates[0]).getTime();
              for (var i = 0; i < inst.dates.length; i++)
                selected =
                  selected ||
                  (inst.dates[i] &&
              ...
              html +=
                '<td data-content="" class="' +
                this._dayClass[useTR] +
                ((dow + firstDay + 6) % 7 >= 5
                  ? " " + this._weekendClass[useTR]
                  : "") + // Highlight weekends
                ...
                (defaultDate.getTime() == printDate.getTime() &&
                  ...
                  ? // Or defaultDate is selected printedDate and defaultDate is cursorDate
                    " " /*+ $.datepick._dayOverClass[useTR]*/
                  : "") + // Highlight selected day		//FixIn: 9.2.1.7
                (unselectable
                  ? " " + this._unselectableClass[useTR]
                ...
                (false
                  ? ""
                  : ' onmouseover="' +
                    "jQuery.datepick._doMouseOver(this,'" + //FixIn: 2023-08-06
                    inst.id +
                    "'," +
                    printDate.getTime() +
                    ')"' +
                    " onmouseout=\"jQuery.datepick._doMouseOut(this,'" +
                    inst.id +
                    "')\"" +
              ...
              html +=
                '<div class="wpbc-cell-box">' + //FixIn: 8.9.4.13
                ...
                (unselectable
                  ? ""
                  : typeof wpbc_show_date_info_top == "function"
                  ? wpbc_show_date_info_top(inst.id, printDate.getTime())
                  : "") +
                "</div>" +
                (empty
                  ? "&#xa0;" // Not showing other months //FixIn:6.0.1.2
                  : unselectable
                  ? "<span>" + printDate.getDate() + "</span>"
                  : "<a>" + printDate.getDate() + "</a>") +
                '<div class="date-content-bottom">' +
                (unselectable
                  ? ""
                  : typeof wpbc_show_date_info_bottom == "function"
      ...
      html +=
        (showStatus
          ? '<div style="clear: both;"></div><div id="' +
            this._statusId[useTR] +
            inst.id +
            '" class="' +
            this._statusClass[useTR] +
            '">' +
            initStatus +
            "</div>"
          : "") +
        ...
        ($.browser_is_supported_datepick.msie &&
        parseInt($.browser_is_supported_datepick.version, 10) < 7 &&
        !inst.inline
          ? '<iframe src="javascript:false;" class="' +
            this._coverClass[useTR] +
            '"></iframe>'
          : "");
      inst.keyEvent = false;
      return html;
    ...
    _generateMonthYearHeader: function (
      ...
      var minDraw = this._daylightSavingAdjust(
    ...
    _adjustInstDate: function (inst, offset, period) {
      ...
      var day =
        Math.min(inst.cursorDate.getDate(), this._getDaysInMonth(year, month)) +
        (period == "D" ? offset : 0);
      inst.cursorDate = this._restrictMinMax(
        inst,
    ...
    _notifyChange: function (inst) {
      ...
      if (onChange)
        onChange.apply(inst.input ? inst.input[0] : null, [
          inst.cursorDate.getFullYear(),
          inst.cursorDate.getMonth() + 1,
          this._daylightSavingAdjust(
            new Date(
              inst.cursorDate.getFullYear(),
              inst.cursorDate.getMonth(),
              1
            )
          ),
          inst,
    ...
    _getMinMaxDate: function (inst, minMax, checkRange) {
      ...
      return checkRange && rangeMin && (!date || rangeMin > date)
        ? rangeMin
        : date;
    },
    ...
    _getRangeMin: function (inst) {
      return this._get(inst, "rangeSelect") && inst.dates[0] && !inst.dates[1]
        ? inst.dates[0]
        : null;
    },
    ...
    _formatDate: function (inst, year, month, day) {
      if (!year) inst.dates[0] = new Date(inst.cursorDate.getTime());
      var date = year
        ? typeof year == "object"
          ? year
          : this._daylightSavingAdjust(new Date(year, month, day))
  ...
  $.fn.datepick = function (options) {
    ...
    if (
      ...
      (options == "isDisabled" || options == "getDate" || options == "settings")
    )
  ...
  $(function () {
    $(document)
      .on("mousedown", $.datepick._checkExternalClick)
      .find("body")
      .append($.datepick.dpDiv); //FixIn: 8.7.11.12
  });
})(jQuery);
